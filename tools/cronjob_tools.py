...
def _format_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = {
        ...
        "probability": job.get("probability", 1.0),
    }
    if job.get("script"):
            ...
            if probability is not None:
                try:
                    ...
                    if not (0.0 <= prob_val <= 1.0):
                        return tool_error("probability must be between 0.0 and 1.0", success=False)
                except (ValueError, TypeError):
                    return tool_error("probability must be a float between 0.0 and 1.0", success=False)
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "probability": {
                "type": "number",
                "default": 1.0,
                "description": "Optional trigger probability between 0.0 and 1.0. Default 1.0 means always trigger when scheduled time arrives. If set to e.g. 0.5, the job has a 50% chance to execute when its due time arrives; if it fails the check, it skips to the next schedule period."
            },
        },
        "required": ["action"]
    }
